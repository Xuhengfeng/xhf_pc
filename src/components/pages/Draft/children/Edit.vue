<template>
  <div class="wrapContent">
    <div class="blockTop">
      <header-bar title="我的草稿"></header-bar>
    </div>
...
export default {
  ...
  methods: {
    ...
    getFormData () {
      ...
      var params = {
        notReadUserIdList: detail.noread.split('-'),
        readUserIdList: detail.read.split('-'),
        id: detail.id
      }
      this.$classAjax.post(this.$URL.classNoticeDetail, params).then(res => {
        this.form.noticeContent = res.noticeContent
        this.form.noticeName = res.noticeName
        this.attachments = res.noticeFileList
        var personlist = res.notReadUserList.map(el => {
          return {
            phone: el.phone,
            sex: el.gender,
            studentId: el.userId,
        ...
        this.approver = personlist
        this.form.persons[0].count = personlist.length
        this.getParentList(personlist)
      })
    },
    // 获取家长列表
    getParentList (value) {
    ...
    submit () {
      this.disabled = true
      this.$classAjax.put(this.$URL.classNoticeUpdate, this.params).then(res => {
